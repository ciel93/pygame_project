...
class BossEnemy(Enemy):
    ...
    def create_pattern(self):
        ...
        if self.pattern_timer > 240: # パターン切替時間を少し短縮
            self.pattern = (self.pattern + 1) % 7 # パターン数を7に増やす
            self.pattern_timer = 0
    ...
    def _laser_sweep(self):
        ...
        if self.pattern_timer % 12 == 0:
            ...
            EnemyBullet(self.enemy_bullets, self.rect.centerx, self.rect.centery, self.player_group, speed=speed, direction=direction, radius=24, length=150, color=(255, 50, 255), bullet_type='laser')

        # 角度を更新して薙ぎ払い
        self.laser_angle += 0.8 * self.laser_sweep_dir # 薙ぎ払う速度を遅くする
    ...
    def _laevateinn_sweep(self):
        ...
        # 弾の生成を8フレームに1回に間引く（負荷軽減のため）
        if self.pattern_timer % 8 == 0:
            # 薙ぎ払いの進行度 (0.0 -> 1.0)
            progress = self.pattern_timer / self.pattern_change_time
            angle_deg = 90 - (80 * progress * self.laevateinn_dir) # 10度 <-> 170度 をスイング
            angle_rad = math.radians(angle_deg)
            direction = pygame.math.Vector2(math.cos(angle_rad), math.sin(angle_rad))

            # 剣を形成する弾を生成
            sword_length = 18 # 剣の長さを構成する弾の数を削減 (35 -> 18)
            for i in range(sword_length):
                dist = i * 28 # 弾の間隔を広げて、見た目の長さを維持 (15 -> 28)
                pos = pygame.math.Vector2(self.rect.center) + direction * dist
                # 剣の先端ほど速く、大きく、赤くなる
                speed = 2.0 + (i / sword_length) * 3.0
                radius = 10 + (i / sword_length) * 12 # 弾のサイズを全体的に大きくする
